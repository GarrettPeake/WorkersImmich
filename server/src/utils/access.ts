/**
 * Access checking utility — Workers-compatible version.
 *
 * Determines if a user/shared-link can access a specific resource
 * by delegating to the AccessRepository.
 *
 * No NestJS imports. Uses plain Error subclasses for access violations.
 */

import type { AuthSharedLink } from 'src/database';
import type { AuthDto } from 'src/dtos/auth.dto';
import { AlbumUserRole, Permission } from 'src/enum';
import type { AccessRepository } from 'src/repositories/access.repository';
import { setDifference, setIsEqual, setIsSuperset, setUnion } from 'src/utils/set';

// ---------------------------------------------------------------------------
// isGranted — checks if current permissions satisfy requested ones
// ---------------------------------------------------------------------------
...
const checkSharedLinkAccess = async (
  ...
  switch (permission) {
    ...
    case Permission.AssetShare: {
      return await access.asset.checkOwnerAccess(
        sharedLink.userId,
        ids,
        false,
      );
...
// ---------------------------------------------------------------------------
// Non-shared-link access checks
// ---------------------------------------------------------------------------

const checkOtherAccess = async (
  access: AccessRepository,
  request: OtherAccessRequest,
): Promise<Set<string>> => {
  ...
  switch (permission) {
    ...
    case Permission.ActivityDelete: {
      const isOwner = await access.activity.checkOwnerAccess(
        auth.user.id,
        ids,
      );
      const isAlbumOwner = await access.activity.checkAlbumOwnerAccess(
        auth.user.id,
    ...
    case Permission.AssetRead: {
      const isOwner = await access.asset.checkOwnerAccess(
        auth.user.id,
        ids,
    ...
    case Permission.AlbumRead: {
      const isOwner = await access.album.checkOwnerAccess(
        auth.user.id,
        ids,
      );
    ...
    case Permission.TimelineRead: {
      const isOwner = ids.has(auth.user.id)
        ? new Set([auth.user.id])
        : new Set<string>();
      const isPartner = await access.timeline.checkPartnerAccess(
        auth.user.id,
        setDifference(ids, isOwner),
      );
      return setUnion(isOwner, isPartner);
...
export const requireElevatedPermission = (auth: AuthDto) => {
  if (!auth.session?.hasElevatedPermission) {
    throw new AccessError(401, 'Elevated permission is required');
  }
};
